...
data/products.md                상품 20종 목록 — 소재 분류용. 답글 링크는 홈(loverebbit.xyz) 고정
...
`variants[].pages = [본문, 답글1, 답글2, ...]`. 2장~ 는 `reply_to_id` 로 순차로 이어붙는다.
장수 제한 없음 — `threads.ts` 가 배열 길이만큼 발행한다.

- **답글은 뭉치지 않는다.** (2026-09-01 지시) 항목이 여럿이면 **항목당 한 장**으로 쪼갠다.
  한 장 38~140자. 한 화면에 들어오는 분량으로 끊는다.
  긴 답글 하나에 다 넣으면 안 읽힌다. 댓글이 많아지는 건 상관없다.
- 본문 끝: 댓글 질문 한 줄 + 해시태그 ≤3. **본문에 URL 넣지 않는다** (도달 깎임).
- **링크는 마지막 장에만.** 중간 답글에는 넣지 않는다.
  **첫 답글에는 절대 링크를 넣지 않는다** (2026-09-03 실측). 본문→첫답글 유지율이
  링크 20.2% / 내용 14.0% 로 갈린다. 본문 1000v 이상에서는 18.0% / 9.0% 로 2배.
  첫 답글만 넘어가면 그 뒤 유지율은 96% 라 링크는 3~4번째 장에서 거의 손해가 없다.
  본문이 뜰 글일수록 4장(본문+내용답글 2장+링크), 작게 끝날 글은 2장.
- **목록형은 `이름 : 한 줄`.** 항목을 문장으로 풀어 설명하지 않는다 (2026-09-01 지시).
  마지막 장(한정 조항)만 문장체 허용.
- **한 줄 24자 이하** (분기 해설 `: ~` 줄만 예외). 마침표로 두 문장을 한 줄에 붙이지 않는다.
  `node scripts/check-lines.mjs "<본문>"` 로 기계 검사. 코퍼스 중앙 17자 기준.
- **링크는 홈 `https://loverebbit.xyz` 하나로 고정** (2026-09-02 지시). 상품 상세 페이지 URL을 직접 걸지 않는다.
  발행할 때 `threads.ts` 가 UTM(`utm_content=<draft-id>`)을 자동으로 붙인다 — 초안에는 맨 URL만 쓴다.
  어느 글이 결제를 만들었는지는 웹앱 `lr_orders.metadata.attribution` 에서 초안 id 로 확인한다.
  `data/products.md` 의 상품 20종 5개 군(연애·마음 / 관계의 끝과 다시 / 일·공부 / 재물 / 삶의 자리)은
  **소재 분류·중복 방지용**으로만 쓰고, 초안의 `product` 필드에 남긴다. 대응 상품이 없는 소재는 답글 없이 본문만.
- **첫 줄은 꺾쇠 제목 고정** — `<~한 사주>` 형태. 훅 문구는 2행으로 내린다 (2026-08-27 결정).
- **가격·판촉 문구 금지.** 본문·답글 어디에도 금액("990원")·할인·특가·선착순·"오늘까지만" 을 쓰지 않는다.
  링크만 걸고 가격은 사이트에서 보게 한다. `check-draft.mjs` 가 기계로 잡는다.
- `{{LINK}}` 플레이스홀더를 쓰면 `LOVEREBBIT_LINK` 로 치환 (비어 있으면 발행 거부). 홈 URL 직접 기입이 기본.
